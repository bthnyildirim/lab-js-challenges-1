...
function multiplyBy(arrayNumbers, multi) {
...
function uniquifyArray(duplicate) {
  ...
  while (i !== duplicate.length) {
    if (
      duplicate.slice(i + 1, duplicate.length - 1).indexOf(duplicate[i]) > -1
    ) {
      duplicate = duplicate.splice(i, 0);
    } else {
      i++;
    }
  }
